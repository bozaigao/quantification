...
def get_previous_trading_day(date_object):
    ...
    while True:
        ...
        if str(date_object) in dates:  # 如果是交易日，则返回该日期
            return date_object
def getHuanShouInfo(date,code):
    browserTab.Page.navigate(url=f"https://www.iwencai.com/stockpick/search?rsh=3&typed=1&preParams=&ts=1&f=1&qs=result_rewrite&selfsectsn=&querytype=stock&searchfilter=&tid=stockpick&w={date} {code}换手率")
    ...
    for div in div_content:
        ...
        if '换手率' in text:
            ...
            stock['huanshou'] = final_stop_time
    return stock        

def get_huanshou_info(date,stocks):
    global batch_size
    data_list = []
    for i in range(len(stocks)):
        ...
        if len(current_batch) == 1:
            stock = getHuanShouInfo(date,current_batch[0]["code"])
            data_list.append({
                    'code': current_batch[0]["code"],
                    'name': current_batch[0]["name"],
                    'huanshou': stock['huanshou']
                })
        else:
            # 添加 '换手率' 文字到搜索文本中
            search_text += '换手率'
            ...
            for index,row in enumerate(table.find_all('tr')):
                ...
                if len(columns) >= 9:  # 确保每行有足够的数据列
                    huanshou = columns[2].find('div', class_='em').text.strip()
                    # 添加到结果列表
                    data_list.append({
...
for item in first_limit_backtest_data[len(new_first_limit_backtest_data):]:
    ...
    for item2 in item['data']:
        if 'isBurst' in item2:
            data.append(item2)
    data_list = get_first_limit_time_info(str(get_previous_trading_day(datetime.strptime(item['date'], '%Y-%m-%d').date())),data)
    ...
    pre_pre_date = get_previous_trading_day(pre_date)
    data_list1 = get_huanshou_info(str(pre_pre_date),data)
    data_list2 = get_huanshou_info(str(pre_date),data)
    for index,item3 in enumerate(data_list1):
        for item4 in data:
            if item3['code'] == item4['code']:
               item4['pre_huanshou'] = data_list1[index]['huanshou']
    for index,item3 in enumerate(data_list2):
        for item4 in data:
            if item3['code'] == item4['code']:
               item4['huanshou'] = data_list2[index]['huanshou']
